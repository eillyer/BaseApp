...
+ (NSString *)toStrWithMoney:(double)money{
    NSString *tempStr = [NSString stringWithFormat:@"%.2lf",money];
    NSNumberFormatter *formatter = [[NSNumberFormatter alloc]init];
    formatter.numberStyle =kCFNumberFormatterCurrencyStyle;
    NSString *newAmount = [formatter stringFromNumber:[NSNumber numberWithDouble:[tempStr doubleValue]]];
    return [newAmount substringWithRange:NSMakeRange(1, newAmount.length-1)];
}
