...
+ (UIImage *)addWatermarkWithBaseImage:(UIImage *)baseImage markImage:(UIImage *)markImage position:(WatermarkPicturePositon)position alpha:(float)alpha {
    ...
    CGSize markSize = markImage.size;
    if (markSize.width>baseSize.width/5) {
        markSize = CGSizeMake(baseSize.width/5, baseSize.width/5);
    }
    CGPoint markPosition = CGPointZero;
    
    UIGraphicsBeginImageContext(baseSize);
    [baseImage drawInRect:CGRectMake(0,0,baseSize.width,baseSize.height)];
    switch (position) {
        case WatermarkPicturePositonTopLeft:
            markPosition = CGPointZero;
            break;
        case WatermarkPicturePositonTopMid:
            markPosition = CGPointMake((baseSize.width - markSize.width)/2, 0);
            break;
        case WatermarkPicturePositonTopRight:
            markPosition = CGPointMake(baseSize.width - markSize.width,0);
            break;
        case WatermarkPicturePositonMidLeft:
            markPosition = CGPointMake(0, (baseSize.height - markSize.height)/2);
            break;
        case WatermarkPicturePositonMid:
            markPosition = CGPointMake((baseSize.width - markSize.width)/2, (baseSize.height - markSize.height)/2);
            break;
        case WatermarkPicturePositonMidRight:
            markPosition = CGPointMake(baseSize.width - markSize.width, (baseSize.height - markSize.height)/2);
            break;
        case WatermarkPicturePositonDownLeft:
            markPosition = CGPointMake(0, baseSize.height - markSize.height);
            break;
        case WatermarkPicturePositonDownMid:
            markPosition = CGPointMake((baseSize.width - markSize.width)/2, baseSize.height - markSize.height);
            break;
        case WatermarkPicturePositonDownRight:
            markPosition = CGPointMake(baseSize.width - markSize.width, baseSize.height - markSize.height);
            break;
        default:
            break;
    }
    [markImage drawInRect:CGRectMake(markPosition.x, markPosition.y, markSize.width, markSize.height) blendMode:kCGBlendModeNormal alpha:alpha];
    // 获取新的图片
    UIImage *newImage =  UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    return newImage;
