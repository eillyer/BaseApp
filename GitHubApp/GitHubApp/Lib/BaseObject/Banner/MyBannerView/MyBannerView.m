...
#import "MyBannerFlowLayout.h"

#define KcellBottom 25
@interface MyBannerView ()<UICollectionViewDataSource,UICollectionViewDelegate>

@property (nonatomic,strong) UIImageView *backgroundImageView;
@property (nonatomic,strong) MyBannerFlowLayout *flowLayout;
...
{
    MyBannerView *cycleScrollView = [[MyBannerView alloc]initWithFrame:frame];
...
-(void)layoutSubviews
{
    ...
    self.pageControl.frame = CGRectMake(self.bounds.size.width/3*2, self.bounds.size.height - self.PageBottom, self.bounds.size.width/3, 10);
    self.flowLayout.itemSize = CGSizeMake(_itemWidth, self.bounds.size.height-KcellBottom);
...

{
    ...
    if ([self.delegate respondsToSelector:@selector(cycleScrollView:atItemAtIndex:)]) {
        [self.delegate cycleScrollView:self atItemAtIndex:[self currentIndex] % self.imgArr.count];
    }
...
{
    MyBannerCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellID forIndexPath:indexPath];
    long itemIndex = (int) indexPath.item % self.imgArr.count;
    NSString *imagePath = self.imgArr[itemIndex];
    
    if ([imagePath hasPrefix:@"http"]||[imagePath isEqualToString:@""]) {
    ...
    [cell setYinyingColor:kBlackColor alpha:0.4 offsetX:3 Y:3 radius:5];
...
-(void)setItemWidth:(CGFloat)itemWidth
{
    ...
    self.flowLayout.itemSize = CGSizeMake(itemWidth, self.bounds.size.height-KcellBottom);
}
...
-(UICollectionView *)collectionView
{
    if(_collectionView == nil)
    {
        ...
        //注册cell
        [_collectionView registerClass:[MyBannerCell class] forCellWithReuseIdentifier:cellID];
...
-(UIPageControl *)pageControl
{
    if(_pageControl == nil)
    {
        ...
        _pageControl.pageIndicatorTintColor = kColorWithHex(@"A4AAB4");
    }
    return _pageControl;
}
-(MyBannerFlowLayout *)flowLayout
{
    if(_flowLayout == nil)
    {
        _flowLayout = [[MyBannerFlowLayout alloc]init];
